...
namespace LWEngine {
	...
	namespace Utils {
		...
		static GLenum ShaderTypeFromString(const std::string& type)
		{
			...
			LWE_CORE_ASSERT(false, "Unknown shader type!");
			return 0;
		}
		...
		: m_FilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			LWE_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					LWE_CORE_ERROR(module.GetErrorMessage());
					LWE_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			LWE_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		LWE_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		LWE_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		LWE_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		LWE_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			LWE_CORE_TRACE("  {0}", resource.name);
			LWE_CORE_TRACE("    Size = {0}", bufferSize);
			LWE_CORE_TRACE("    Binding = {0}", binding);
			LWE_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
