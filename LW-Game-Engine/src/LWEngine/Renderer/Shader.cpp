...
namespace LWEngine {
	Shader::Shader(const std::string& vertexSrc, const std::string& fragmentSrc)
	{
		...
		if (isCompiled == GL_FALSE)
		{
			...
			LWE_CORE_ASSERT(false, "COMPILATION_FAILED::VERTEX_SHADER");
			return;
		}
		...
		if (isCompiled == GL_FALSE)
		{
			...
			LWE_CORE_ERROR("{0}", infoLog.data());
			LWE_CORE_ASSERT(false, "COMPILATION_FAILED::FRAGMENT_SHADER");
			return;
		}
	...
	void Shader::UploadUniformFloat(const std::string& name, GLfloat uniform)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
